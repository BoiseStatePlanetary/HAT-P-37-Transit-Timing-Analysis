...
def read_data_file(file_name):
    ...
    return np.column_stack((bjd_tdb, phot_data[:, 1], ))
...
def combine_data_and_fit(data_files):
    ...
    for i, file in enumerate(data_files):
        if i == 0:
            photdata = read_data_file(file)
            time = photdata["BJD_TDB"]
            flux = photdata["flux"]
            error = photdata["error"]  
        else:
             photdata = read_data_file(file)
             time.append(photdata["BJD_TDB"])
             flux.append(photdata["flux"])
             error.append(photdata["error"])                         
    print(time)
    
    all_times = np.array(time)
    all_fluxes = np.array(flux)
    all_error = np.array(error)

    # test out a sine fit
    guess_mean = np.mean(all_fluxes)
    guess_std = 3*np.std(all_fluxes)/(2**0.5)/(2**0.5)
    guess_phase = 0
    ...
    optimize_func = lambda x: x[0]*np.sin(x[1]*all_times+x[2]) + x[3] - all_fluxes
    est_amp, est_freq, est_phase, est_mean = leastsq(optimize_func, [guess_amp, guess_freq, guess_phase, guess_mean])[0]

    # recreate the fitted curve using the optimized parameters
    data_fit = est_amp*np.sin(est_freq*all_times+est_phase) + est_mean

    # recreate the fitted curve using the optimized parameters

    fine_t = np.arange(0,max(all_times),0.1)
    data_fit=est_amp*np.sin(est_freq*fine_t+est_phase)+est_mean

    plt.plot(all_times, all_fluxes, '.')
    plt.plot(all_times, data_first_guess, label='first guess')
