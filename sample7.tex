...
Using the orbital period from Chen et. al. and the tools in the lightkurve package, we have been able to separate the transit light curves for HAT-P-37 b from the variability of the EB. 
...
Tess lightcurve fitting process (fit by individual sector):
\begin{itemize}
    \item Use box least squares fitting to assess if the sector is contaminated by the EB (see above)
    \item if the sector is NOT contaminated 
    \begin{itemize}
        \item use the Carter model to fit each transit 
        \item extract a midtime for each transit in the sector
    \end{itemize}
    \item if the sector is contaminated
    \begin{itemize}
        \item create a known transit mask and apply
        \item fold the sector to the period of the binary star
        \item bin the data to the original sector cadence
        \item interpolate the full w/o transit(currently all code fits folded) time series to the folded/binned data w/o the transit
        \item normalize the full w/ transit time series to the interpolated series
        \item chunk the series by the transit orbital period 
        \item fit each chunk using the carter model
        \item extract a midtime for each transit in the sector (approx. 10 per sector)
